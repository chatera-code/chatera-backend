...
def store_tables_in_mysql(doc_id: str, filename: str, tables: List[Dict]):
    ...
    with engine_mysql.connect() as connection:
        try:
            ...
            db_for_update: Session = SessionLocal()
            try:
                doc_to_update = db_for_update.query(Document).filter(Document.doc_id == doc_id).first()
                if doc_to_update:
                    doc_to_update.db_name = db_name
                    db_for_update.commit()
                    logger.info(f"Updated document {doc_id} with db_name: {db_name}")
                else:
                    logger.warning(f"Could not find document {doc_id} to update db_name.")
            finally:
                db_for_update.close()
