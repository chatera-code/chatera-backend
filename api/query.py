...
async def query_documents(
    ...
    if edge_response and edge_response[0]:
        ...
        for match in edge_response[0]:
            ...
            # Load the graph if it hasn't been already
            if matched_doc_id not in loaded_graphs:
                graph = KnowledgeGraph.load(matched_doc_id, GRAPH_DIR)
                if graph:
                    loaded_graphs[matched_doc_id] = graph
            
            # Find the edge in the loaded graph and get its nodes
        ...
        # Expand context for each document's graph from the initial nodes
        contexts = []
        for doc_id, node_ids in initial_nodes_by_doc.items():
            graph = loaded_graphs[doc_id]
            # Assumes expand_context_from_nodes is a method on KnowledgeGraph
